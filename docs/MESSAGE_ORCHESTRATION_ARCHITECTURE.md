...
Production remains fail closed: consumer acknowledgment, follow-up email, sequence scheduler, auto-send, and carrier SMS are disabled. The Phase 6 migration was not applied because Preview database identity could not be proven distinct from Production; runtime features that require it remain inactive. Existing genuine internal alerts continue through the proven durable `lead_notifications` outbox.

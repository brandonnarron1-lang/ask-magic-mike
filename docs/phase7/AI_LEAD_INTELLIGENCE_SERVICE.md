...
Production uses `AI_TIMEOUT_MS=20000`, an encrypted Production-only value within the adapter's existing 20-second hard cap. This accommodates the verified provider latency while preserving the bounded two-attempt policy, deterministic fallback, cost ceilings, and no-send/no-assignment controls. The current-release acceptance persisted one provider-backed intelligence row and usage telemetry for a suppressed synthetic lead without changing lead state or contact timestamps.

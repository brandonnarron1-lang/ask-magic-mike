...
Phase 7 adds narrowly scoped names: `RESEND_WEBHOOK_SECRET`, `QA_EMAIL_MAX_SENDS`, `AI_ASYNC_COPILOT_ENABLED`, `AI_ASYNC_WORKER_ENABLED`, and `AI_TIMEOUT_MS`.
...
The first current-release provider attempt exhausted the existing eight-second request window and returned the documented deterministic fallback without mutating lead or communication state. `AI_TIMEOUT_MS=20000` was then added as an encrypted Production-only value, within the adapter's existing hard cap. The key, model, retry count, token/cost ceilings, feature flags, and automatic-action controls were not changed.
...
The existing key successfully completed a deployed Responses API acceptance on synthetic, suppressed data after the bounded timeout adjustment. Deployment `dpl_7uQC5a9xudCNAN1HEAiBWdBZ7iC9` returned strict structured output in 7,624 ms and durably recorded intelligence and usage telemetry. No duplicate OpenAI key was created. Production keeps advisory intelligence and persistence enabled while asynchronous workers and all automatic actions remain disabled.

...
| Rate limit | `DATABASE_URL`, `LEAD_RATE_LIMIT_PER_MINUTE`, `RATE_LIMIT_HASH_SECRET`, `RATE_LIMIT_EMERGENCY_MEMORY` | server | durable Neon abuse control; dedicated HMAC secret preferred, documented server-secret fallback allowed; emergency memory mode must be explicit |

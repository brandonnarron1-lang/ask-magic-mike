...
- Pre-reconciliation verification passed 2 focused files / 24 tests, 215 full
  ...
  with no overflow, console finding, or application POST. The reconciled final
  PR head must repeat local, exact Node 24 CI, immutable Preview, and protected
  no-write QA; exact immutable identifiers belong in the PR evidence record.

...
export type AnalyticsEventWrite = {
  ...
  /**
   * Pseudonymous UUID generated by a public funnel. If the visitor later submits,
   * the same UUID becomes `sessions.id`; until then it remains a protected,
   * pseudonymous analytics property and never creates a canonical session.
   */
...
export class NeonAnalyticsEventRepository {
  ...
  async record(event: AnalyticsEventWrite): Promise<boolean> {
    ...
    const sessionId = validUuid(event.sessionId);
    const properties = safeAnalyticsProperties(event.properties);
    const funnelSessionId = validUuid(event.funnelSessionId);
    if (funnelSessionId) properties.funnel_session_id = funnelSessionId;
    const utmSource = safeAnalyticsDimension(event.utmSource);
    const utmMedium = safeAnalyticsDimension(event.utmMedium);
    const utmCampaign = safeAnalyticsDimension(event.utmCampaign);
    const userAgent = coarseAnalyticsUserAgent(
      event.userAgent,
      properties.device_category ?? properties.deviceType,
    );
    const params = [
      sessionId,

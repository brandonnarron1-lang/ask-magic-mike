...
function renderLeadAlertWithMode(
  ...
  const visualHeader = !shouldRenderLeadAlertVisual()
    ...
    : visualVersion === "legacy_v2"
      ...
      : `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#080808"><tr><td background="${html(visualAssetUrl(visual.backgroundAssetPath))}" style="background-color:#080808;background-image:url('${html(visualAssetUrl(visual.backgroundAssetPath))}');background-position:center;background-size:cover"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="table-layout:fixed"><tr><td style="padding:18px 8px 18px 18px;vertical-align:middle;overflow-wrap:anywhere"><img src="${html(visualAssetUrl("/images/ask-magic-mike/our-town-properties-logo.webp"))}" width="136" alt="Our Town Properties, Inc." style="display:block;width:136px;max-width:100%;height:auto;border:0"/><p style="margin:8px 0 3px;color:#f4ead4;font-size:11px;font-weight:700;letter-spacing:.16em">ASK MAGIC MIKE</p><p style="margin:0;color:${html(visual.accent)};font-size:11px;font-weight:700;letter-spacing:.08em">${html(visual.eyebrow)}</p></td><td width="90" align="right" valign="bottom" style="padding:12px 10px 12px 0"><img src="${html(visualAssetUrl("/images/ask-magic-mike/brand-pack-v2/mike-avatar-circle-256.webp"))}" width="80" alt="Mike Eatmon, Broker and REALTOR" style="display:block;width:80px;max-width:80px;height:80px;border:0;border-radius:999px"/></td></tr></table></td></tr></table>`;
  const htmlBody = `<div style="margin:0;padding:0;background:#090909;font-family:Arial,sans-serif;color:#f8fafc;line-height:1.5;overflow-wrap:anywhere"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:560px;margin:0 auto;background:#101010;border:1px solid ${html(visual.accent)};box-sizing:border-box"><tr><td>${visualHeader}<div style="padding:24px;overflow-wrap:anywhere"><p style="margin:0 0 6px;color:${html(visual.accent)};font-weight:700;letter-spacing:.08em;font-size:12px">${html(visual.eyebrow)}</p><h1 style="margin:0 0 8px;color:#fff;font-size:25px">${html(tag)} ${html(label)}</h1><p style="margin:0 0 20px"><strong>${html(recordStatus)}</strong></p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;table-layout:fixed;border-top:1px solid #4b3b14">${rows.map(([key, value]) => `<tr><td style="padding:8px 14px 8px 0;color:#d4a72c;font-weight:700;vertical-align:top;width:38%;overflow-wrap:anywhere">${html(key)}</td><td style="padding:8px 0;color:#f8fafc;overflow-wrap:anywhere;word-break:break-word">${html(value)}</td></tr>`).join("")}</table><p style="margin:20px 0"><a href="${html(leadCenterUrl)}" style="display:inline-block;background:${html(visual.accent)};color:#fff;padding:12px 18px;text-decoration:none;font-weight:700">Open secure Lead Center</a></p><p style="color:#cbd5e1;font-size:12px">Lead ID: ${html(input.leadId)}<br/>Correlation ID: ${html(input.correlationId)}</p><p style="color:#cbd5e1">Next action: ${html(nextAction)}</p><p style="color:#94a3b8;font-size:12px">Not a survey.</p></div></td></tr></table></div>`;

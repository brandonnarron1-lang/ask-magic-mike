...
describe("InMemoryRateLimitStore", () => {
  ...
  it("resets the window when windowMs has elapsed", () => {
    ...
    // Window is 1ms — next call should start a fresh window
...
describe("checkRateLimit (legacy function)", () => {
  it("allows first request", () => {
    const result = checkRateLimit("test-unique-key-1", 10, 60_000);
    expect(result.allowed).toBe(true);
  });
...
describe("rateLimitKey", () => {
  ...
  it("trims leading/trailing whitespace and takes the first IP in X-Forwarded-For", () => {
    expect(rateLimitKey("1.2.3.4, 5.6.7.8")).toBe("1.2.3.4");
  });

  it("returns 'anonymous' when IP header is null (absent)", () => {

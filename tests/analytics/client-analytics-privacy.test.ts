...
describe("client analytics privacy boundary", () => {
  ...
  it("keeps lead-created visible to consented external analytics without duplicating the server ledger", () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(null, { status: 202 }));
    vi.stubGlobal("fetch", fetchMock);
    const browser = window as TestAnalyticsWindow;
    browser.ammDataLayer = [];
  ...
  it("keeps widget conversion visible to consented external analytics without duplicating the server ledger", () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(null, { status: 202 }));
    vi.stubGlobal("fetch", fetchMock);
    const browser = window as TestAnalyticsWindow;
    browser.ammDataLayer = [];

...
const pr212AfterPr211Rehearsal = readDoc(
  "phase9/PR212_AFTER_PR211_INTEGRATION_REHEARSAL.md"
);
...
const pr211SealedHead =
  "6eacc33d16e34897c97288e48cd736433a3d9e15";
const pr212IndependentHead =
...
describe("current release-authority documentation", () => {
  ...
  it("binds the isolated PR #212 compatibility rehearsal to the sealed PR #211 and independent PR #212 heads", () => {
    expect(pr212AfterPr211Rehearsal).toContain(pr211SealedHead);
    expect(pr212AfterPr211Rehearsal).toContain(pr212IndependentHead);
    expect(pr212AfterPr211Rehearsal).toContain(
      "rescue/amm-pr212-rehearsal-pre-pr211-ledger-sync-20260824-0649"
    );
    expect(pr212AfterPr211Rehearsal).toMatch(
      /rehearsal remains a separate[\s\S]*cannot be used as release authority/i
    );
  });

  it("keeps PR #212 application and WordPress activation outside rehearsal authority", () => {
    expect(pr212AfterPr211Rehearsal).toContain(crossDomainGate);
    expect(pr212AfterPr211Rehearsal).toContain(
      "APPROVE PHASE 9 OUR TOWN BASIC CONSENT BRIDGE 1.2.0 INSTALLATION, LEGACY GTM REMOVAL, AND CONTROLLED RUNTIME QA"
    );
    expect(pr212AfterPr211Rehearsal).toMatch(
      /No Vercel environment value[\s\S]*WordPress\/GTM\/GA4 mutation/i
    );
  });

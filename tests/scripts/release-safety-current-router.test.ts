...
describe("release safety current-router coverage", () => {
  ...
  it("requires the mutation-free public-funnel browser acceptance contract", () => {
    ...
    expect(scanner).toContain("tests/e2e/no-write-preview-interception.ts");
    expect(scanner).toContain("tests/e2e/preview-test-config.ts");
    expect(scanner).toContain('page.route("**/api/**"');
    expect(scanner).toContain('new Set(["POST", "PUT", "PATCH", "DELETE"])');
    expect(scanner).toContain("unexpectedMutations.push");
